...
if {$UseSemitcp > 0} {
	...
	if {$argc == 8} {
		...
		CMUPriQueue	 set CongestionThreshold_ [lindex $argv 6]
		...
		CMUPriQueue	 set CongestionThreshold_ 1
		Mac/802_11 set K_ 1
...
for {set i 0} {$i < $val(nn) } {incr i} {     ;# Create the nodes
	...
	set rt($i) [$node_($i) set dsr_agent_]
...
proc create_tcp_connection {id src dst} {
	...
	if {$UseSemitcp > 0} {
		...
		set rt($src) [$node_($src) set dsr_agent_]
		$rt($src) aodv-get-semitcp $tcp_($id)
		# 4.2 AODV---->TcpSink
		set rt($dst) [$node_($dst) set dsr_agent_]
